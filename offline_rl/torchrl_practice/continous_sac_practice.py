...
from offline_rl.torchrl_practice.env_util import record_policy_video, setup_pendulum_with_wrapper, setup_visual_minecraft_with_wrapper, setup_visual_minecraft
from offline_rl.torchrl_practice.networks import ActorNet, ActorNet2, QNet, QNet2, VNet
...
from torchrl.objectives import ClipPPOLoss, DiscreteSACLoss, SACLoss, SoftUpdate
...
if __name__ == "__main__":
    ...
    env = setup_pendulum_with_wrapper(device=device)

    actor_net = ActorNet2().to(device)
    policy = ProbabilisticActor(
        ...
        distribution_kwargs={
            "low": env.action_spec.low, "high": env.action_spec.high
        },
    ...
    def create_vectorized_env():
        ...
        return ParallelEnv(
            ...
            create_env_fn=lambda: setup_pendulum_with_wrapper(device=torch.device("cpu")),
        )
    ...
    logger = WandbLogger(
        exp_name="sac-training-pendulum",
    ...
    for i, tensordict_data in tqdm(enumerate(collector)):
        ...
        if i % 10 == 0:
            ...
            with set_exploration_type(ExplorationType.DETERMINISTIC), torch.no_grad():
                ...
                try:
                    render_env_fn = setup_pendulum_with_wrapper
                    video_frames, cummulative_reward = record_policy_video(policy, max_steps=10, seed=42, device=device, render_env_fn=render_env_fn)
