...
from offline_rl.stable_baseline_practice.discrete_sac_policies import DiscreteActor, DiscreteCnnPolicy, DiscreteCritic, DiscreteMlpPolicy, DiscreteMultiInputPolicy, DiscreteSACPolicy
...
class DiscreteSAC(OffPolicyAlgorithm):
    """
    Discrete Soft Actor-Critic (DSAC)
    ...
    policy_aliases: ClassVar[dict[str, type[BasePolicy]]] = {
        "DiscreteMlpPolicy": DiscreteMlpPolicy,
        "DiscreteCnnPolicy": DiscreteCnnPolicy,
        "DiscreteMultiInputPolicy": DiscreteMultiInputPolicy,
    }
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                next_log_probs = self.actor.action_log_prob(replay_data.next_observations)  # [batch_size, num_actions]
                next_probs = next_log_probs.exp()  # [batch_size, num_actions]
                # Compute the next Q values: min over all critics targets
                next_q_values_all = self.critic_target(replay_data.next_observations)
            ...
            current_q_values = []
            for current_q in self.critic(replay_data.observations):
