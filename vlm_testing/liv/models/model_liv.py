...
class LIV(nn.Module):
    def __init__(self, modelid="RN50", device="cuda",
        ...
        # Note: scratch argument may not be available in all CLIP versions
        try:
            model, cliptransforms = clip.load(modelid, device=self.device, scratch=scratch, jit=False)
        except TypeError:
            # If scratch argument is not supported, try without it
            if scratch:
                raise ValueError("scratch=True is not supported in this CLIP version. Please install a CLIP version that supports the scratch argument.")
