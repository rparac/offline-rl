...
imgs = []
imgs_tensor = []
# Load transformed episode step files (steps 0-9, 10 total)
frame_files = sorted(glob.glob("sample_video/transformed_episode_0_step_*.png"))
...
with torch.no_grad():
    embeddings = model(input=imgs_tensor.to(device), modality="vision")
...
fig.savefig(f"liv_reach_diamond.png", bbox_inches='tight')
...
ani.save(f"liv_reach_diamond.gif", dpi=100, writer=PillowWriter(fps=25))
plt.close()